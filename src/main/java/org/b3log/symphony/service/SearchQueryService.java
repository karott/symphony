 ...
 * <p>
 * Uses
 * <a href="https://www.elastic.co/products/elasticsearch">Elasticsearch</a> as
 * the underlying engine. Uses <a href="https://www.algolia.com">Algolia</a> as
...
public class SearchQueryService {
    ...
    public JSONObject searchElasticsearch(final String type, final String keyword, final int currentPage, final int pageSize) {
        try {
            ...
            final HttpResponse response = HttpRequest.post(Symphonys.ES_SERVER + "/" + SearchMgmtService.ES_INDEX_NAME + "/" + type + "/_search").bodyText(reqData.toString()).contentTypeJson()
                    .timeout(5000).send();
    ...
    public JSONObject searchAlgolia(final String keyword, final int currentPage, final int pageSize) {
        ...
        while (retries <= maxRetries) {
            ...
            try {
                ...
                params.put("params", "query="	+ URLs.encode(keyword) + "&getRankingInfo=1&facets=*&attributesToRetrieve=*&highlightPreTag=%3Cem%3E" + "&highlightPostTag=%3C%2Fem%3E"
                        + "&facetFilters=%5B%5D&maxValuesPerFacet=100" + "&hitsPerPage=" + pageSize + "&page=" + (currentPage - 1));
                final HttpResponse response = HttpRequest.post("https://" + host + "/1/indexes/" + index + "/query").header("X-Algolia-API-Key", key).header("X-Algolia-Application-Id", appId)
                        .bodyText(params.toString()).contentTypeJson().connectionTimeout(5000).timeout(5000).send();
                response.charset("UTF-8");
